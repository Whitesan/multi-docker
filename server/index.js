...
async function connectWithRetry(client, retries = 5) {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      console.error(`Postgres not ready (${i + 1}/${retries}). Retrying in 3s...`);
...
// ✅ Try to connect and initialize the DB
connectWithRetry(pgClient).catch((err) => {
  console.error(err);
  process.exit(1); // Optional: fail container if DB never connects
});
